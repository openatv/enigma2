...
def FileEntryComponent(name, absolute=None, isDir=False, isLink=False):
	...
	res.append((eListboxPythonMultiContent.TYPE_TEXT, int(x), int(y), int(w - x), int(h), 0, RT_HALIGN_LEFT, name))
	if isLink:
	...
	if png is not None:
		...
		res.append((eListboxPythonMultiContent.TYPE_PIXMAP_ALPHABLEND, int(x), int(y), int(w), int(h), png, None, None, BT_SCALE))
		if link_png is not None:
			res.append((eListboxPythonMultiContent.TYPE_PIXMAP_ALPHABLEND, int(x), int(y), int(w), int(h), link_png, None, None, BT_SCALE))
...
def MultiFileSelectEntryComponent(name, absolute=None, isDir=False, isLink=False, selected=False):
	...
	res.append((eListboxPythonMultiContent.TYPE_TEXT, int(x), int(y), int(w), int(h), 0, RT_HALIGN_LEFT, name))
	...
	if not name.startswith('<'):
		...
		if selected is False:
			...
			res.append((eListboxPythonMultiContent.TYPE_PIXMAP_ALPHABLEND, int(x), int(y), int(w), int(h), icon, None, None, BT_SCALE))
		else:
