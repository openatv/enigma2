...
class Change_volume(ConfigListScreen, Screen):
	...
	def yellowPressed(self):
		global offset
		offset  = (int(self.offset.value)) * -1
		self.offset.value = str(offset)
...
class Volume_Config(ConfigListScreen, Screen):
	...
	def yellowPressed(self):
		...
		config.Volume.AC3_vol.value  = (int(config.Volume.AC3_vol.value)) * -1
		self["config"].list = self.Clist
		self["config"].l.setList(self.Clist)
	...
	def cancel(self):
		config.Volume.Enabled.value = self.oldEnable
		config.Volume.AC3_vol.value = self.oldOffset
		config.Volume.save()
		self.close()
