	...
	def updateList2(self):
		...
		for line in f.readlines():
			...
			if not search('sd[a-z][1-9]',device) and not search('mmcblk[0-9]p[1-9]',device):
				continue
			if getMachineBuild() in ('vuuno4k', 'vuultimo4k', 'vusolo4k', 'hd51', 'hd52', 'dm820', 'dm7080', 'sf4008', 'dm900', 'gbuhdquad', 'revo4k', 'force3uhd', 'force3uhdplus') and search('mmcblk0p[1-9]',device):
				continue
			if device in list2:
				continue
