...
from enigma import eCableScan

class CableScan:
	def __init__(self, text, progressbar, scanTuner, scanNetwork, scanFrequency, scanSymbolRate, keepNumbers):
		self.text = text;
	...
	def execBegin(self):
		...
		self.scan = eCableScan(self.scanNetwork, self.scanFrequency, self.scanSymbolRate, self.keepNumbers)
...
class CableScanStatus(Screen):
	...
	def __init__(self, session, scanTuner, scanNetwork, scanFrequency, scanSymbolRate, keepNumbers):
	...
	def doServiceScan(self):
		self["scan"] = CableScan(self["scan_state"], self["scan_progress"], self.scanTuner, self.scanNetwork, self.scanFrequency, self.scanSymbolRate, self.keepNumbers)
...
class CableScanScreen(ConfigListScreen, Screen):
	...
	def startScan(self):
		self.session.open(CableScanStatus, scanTuner = int(self.scan_nims.value), scanNetwork = config.plugins.CableScan.networkid.value, scanFrequency = config.plugins.CableScan.frequency.value * 1000, scanSymbolRate = config.plugins.CableScan.symbolrate.value * 1000, keepNumbers = config.plugins.CableScan.keepnumbering.value)
