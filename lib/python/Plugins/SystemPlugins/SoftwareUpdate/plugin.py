...
class UpdatePlugin(Screen):
	...
	def __init__(self, session, args = None):
		...
		self.status = StaticText(_("Upgrading STB-BOX... Please wait"))
	...
	def ipkgCallback(self, event, param):
		if event == IpkgComponent.EVENT_DOWNLOAD:
			...
			if self.updating:
				...
				self.activityTimer.stop()
				self.activityslider.setValue(0)
				error = _("your STB-BOX might be unusable now. Please consult the manual for further assistance before rebooting your STB-BOX.")
				if self.packages == 0:
					error = _("No packages were upgraded yet. So you can check your network and try again.")
				if self.updating:
					error = _("Your STB-BOX isn't connected to the internet properly. Please check it and try again.")
	...
	def exit(self):
		if not self.ipkg.isRunning():
			if self.packages != 0 and self.error == 0:
				self.session.openWithCallback(self.exitAnswer, MessageBox, _("Upgrade finished. Do you want to reboot your STB-BOX?"))
			else:
				self.close()
