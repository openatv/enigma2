...
class InstallWizard(Screen, ConfigListScreen):
	...
	def __init__(self, session, args = None):
		...
		if self.index == self.STATE_UPDATE:
			...
			modes = {"19e": _("Astra1"), "23e": _("Astra3"), "19e-23e": _("Astra1 Astra3"), "19e-23e-28e": _("Astra1 Astra2 Astra3"), "13e-19e-23e-28e": _("Astra1 Astra2 Astra3 Hotbird")}
			self.channellist_type = ConfigSelection(choices = modes, default = "19e")
			self.createMenu()
		elif self.index == self.STATE_CHOISE_SOFTCAM:
			self.enabled = ConfigYesNo(default = True)
			modes = {"cccam": _("default (CCcam)"), "scam": _("scam")}
	...
	def createMenu(self):
		...
		if self.index == self.STATE_UPDATE:
			...
			if self.enabled.value:
				self.list.append(getConfigListEntry(_("Channellist type"), self.channellist_type))
		elif self.index == self.STATE_CHOISE_SOFTCAM:
	...
	def run(self):
		if self.index == self.STATE_UPDATE:
			...
			self.session.open(InstallWizardIpkgUpdater, self.index, _('Please wait downloading channellist'), IpkgComponent.CMD_REMOVE, {'package': 'enigma2-plugin-settings-henksat-' + self.channellist_type.value})
