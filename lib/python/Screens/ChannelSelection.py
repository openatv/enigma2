...
class ChannelContextMenu(Screen):
	...
	def setStartupService(self):
		config.servicelist.startupservice.setValue(self.csel.getCurrentSelection().toString())
		path = ''
		for i in self.csel.servicePath:
			path += i.toString()
			path += ';'
		config.servicelist.startuproot.setValue(path)
		config.servicelist.startupmode.setValue(config.servicelist.lastmode.getValue())
		config.servicelist.save()
		configfile.save()
		self.close()

	def unsetStartupService(self):
		config.servicelist.startupservice.setValue('')
		config.servicelist.save()
		configfile.save()
		self.close()
	...
	def removeParentalProtection(self, service):
		self.session.openWithCallback(boundFunction(self.pinEntered, service.toCompareString()), PinInput, pinList = [config.ParentalControl.servicepin[0].getValue()], triesEntry = config.ParentalControl.retries.servicepin, title = _("Enter the service pin"), windowTitle = _("Change pin code"))
...
class ChannelSelection(ChannelSelectionBase, ChannelSelectionEdit, ChannelSelectionEPG, SelectionEventInfo):
	...
	def __init__(self, session):
		...
		if config.servicelist.startupservice.getValue() and config.servicelist.startuproot.getValue():
			config.servicelist.lastmode.setValue(config.servicelist.startupmode.getValue())
			if config.servicelist.lastmode.getValue() == "tv":
				config.tv.lastservice.setValue(config.servicelist.startupservice.getValue())
				config.tv.lastroot.setValue(config.servicelist.startuproot.getValue())
			elif config.servicelist.lastmode.getValue() == "radio":
				config.radio.lastservice.setValue(config.servicelist.startupservice.getValue())
				config.radio.lastroot.setValue(config.servicelist.startuproot.getValue())
	...
	def setModeTv(self):
		...
		self.lastservice = config.tv.lastservice
		self.lastroot = config.tv.lastroot
		config.servicelist.lastmode.setValue("tv")
		self.setTvMode()
		self.setMode()
	...
	def setModeRadio(self):
		...
		if config.usage.e1like_radio_mode.getValue():
			...
			config.servicelist.lastmode.setValue("radio")
			self.setRadioMode()
			self.setMode()
	...
	def channelSelected(self):
		ref = self.getCurrentSelection()
		try:
			doClose = not config.usage.servicelistpreview_mode.getValue() or ref == self.session.nav.getCurrentlyPlayingServiceReference()
		except:
			doClose = False
	...
	def saveRoot(self):
		...
		if path and path != self.lastroot.getValue():
			self.lastroot.setValue(path)
			self.lastroot.save()

	def restoreRoot(self):
		tmp = [x for x in self.lastroot.getValue().split(';') if x != '']
	...
	def saveChannel(self, ref):
		...
		if refstr != self.lastservice.getValue():
			self.lastservice.setValue(refstr)
...
########## RDS Radiotext / Rass Support END
	...
	def saveRoot(self):
		...
		if path and path != config.radio.lastroot.getValue():
			config.radio.lastroot.setValue(path)
			config.radio.lastroot.save()

	def restoreRoot(self):
		tmp = [x for x in config.radio.lastroot.getValue().split(';') if x != '']
		current = [x.toString() for x in self.servicePath]
		if tmp != current or self.rootChanged:
			cnt = 0
			for i in tmp:
		...
		if self.movemode:
			...
			if not cur_root or not (cur_root.flags & eServiceReference.isGroup):
				...
				if playingref is None or playingref != ref:
					self.session.nav.playService(ref)
					config.radio.lastservice.setValue(ref.toString())
