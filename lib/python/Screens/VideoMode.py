...
class VideoSetup(Screen, ConfigListScreen):
	...
	def createSetup(self):
		...
		if config.av.videoport.getValue() in ('HDMI', 'YPbPr', 'Scart-YPbPr') and not path.exists(resolveFilename(SCOPE_PLUGINS)+'SystemPlugins/AutoResolution'):
			...
			self.list.append(getConfigListEntry(_("Allow 25Hz/30Hz"), config.av.autores_all_res,_("With this option enabled these reflash rates will be used. (please note not all TV's support these rates).")))
