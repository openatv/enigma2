...
arg=${{bootargs}} logo=osd0,loaded,0x7f800000 vout=1080p50hz,enable hdmimode=1080p50hz fb_width=1280 fb_height=720 panel_type=lcd_4"""
			...
			# 1. Filter out existing SDcard Slot entries
			new_lines = []
			skip_block = False
			for line in lines:
			...
			# 2. Insert new slots before the last block (usually Recovery)
		...
		def formatDevice():
			...
			if exists(TARGET_DEVICE):
				...
				# Create nomount files early to prevent automount popups
				...
				for i in range(numSlots):
					...
					cmdlist.append(f"/usr/sbin/parted --script {TARGET_DEVICE} mkpart {DEVICE_LABEL} ext4 {start} {end}")
	...
	def deviceSelectionCallback(self, selection):
		if selection:
			...
			# Get base device name by stripping partition number (e.g., mmcblk1p1 -> mmcblk1)
			devicePath = self.deviceData[selection][0]  # e.g., /dev/mmcblk1p1
			baseDevice = devicePath.rstrip('0123456789')  # Remove trailing digits
			if baseDevice.endswith('p'):  # Remove 'p' from mmcblk1p
				baseDevice = baseDevice[:-1]

			print(f"[GPTSlotManager] devicePath={devicePath}, baseDevice={baseDevice}")
			diskSize = self.partitionSizeGB(baseDevice)
			print(f"[GPTSlotManager] diskSize={diskSize}GB")
