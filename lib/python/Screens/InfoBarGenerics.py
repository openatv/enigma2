...
class InfoBarINFOpanel:
	...
	def selectRedKeytask(self):
		...
		if os.path.isfile("/usr/lib/enigma2/python/Plugins/Extensions/WebBrowser/browser.pyo"):
			isWEBBROWSER = True
		if os.path.isfile("/usr/lib/enigma2/python/Plugins/Extensions/HbbTV/plugin.pyo") and getBoxType().startswith('vu') or getBoxType().startswith('venton') or getBoxType().startswith('gb') or getBoxType().startswith('odin'):
			isHBBTV = True
