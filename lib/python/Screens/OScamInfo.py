...
from Components.config import config, ConfigYesNo, ConfigSelectionNumber
...
# TODO: MOVE TO UsageConfig.py
config.oscaminfo.autoupdateFull = ConfigYesNo(default=False)
config.oscaminfo.intervallFull = ConfigSelectionNumber(min=1, max=600, stepwidth=1, default=10, wraparound=True)
...
class OScamOverview(Screen, OScamGlobals):
	...
	skin = """
		<screen name="OScamInfoOverview" position="center,center" size="1950,1080" backgroundColor="#10101010" title="OScamInfo Overview" flags="wfNoBorder" resolution="1920,1080">
			<ePixmap pixmap="/tmp/oscam.svg" position="15,15" size="80,80"/>
			...
			<widget source="outlist" render="Listbox" position="15,210" size="1890,600" backgroundColor="grey" scrollbarMode="showOnDemand" >
				<convert type="TemplatedMultiContent">
					...
					MultiContentEntryText(pos=(1360,0), size=(218,60), font=0, flags=RT_HALIGN_CENTER|RT_VALIGN_CENTER, color=0x000000, color_sel=0x000000, backcolor=MultiContentTemplateColor(11), backcolor_sel=MultiContentTemplateColor(11), text=8),  # LB Value/Reader
					MultiContentEntryText(pos=(1580,0), size=(138,60), font=0, flags=RT_HALIGN_CENTER|RT_VALIGN_CENTER|RT_WRAP, color=0x000000, color_sel=0x000000, backcolor=MultiContentTemplateColor(11), backcolor_sel=MultiContentTemplateColor(11), text=9),  # Online+Idle
					...
					], "fonts": [gFont("Regular",21)], "itemHeight":60
					}
				</convert>
			</widget>
	...
	def __init__(self, session):
		...
		self["actions"] = ActionMap(["OkCancelActions", "ColorActions", "DirectionActions", "MenuActions"],
							{"ok": self.exit,
							"cancel": self.exit,
							"up": self.keyPage_Down,
							"down": self.keyPage_Up,
							"right": self.keyPage_Down,
							"left": self.keyPage_Up,
							"menu": self.key_menu,
							"blue": self.key_blue
							}, -1)
	...
	def onLayoutFinished(self):
		try:  # function is not available in older image releases
			self["outlist"].setSelectionEnable(False)
		except Exception:
			pass
		self.handleAutoupdate(self.updateOScamData)
	...
	def handleAutoupdate(self, update=None):
		if config.oscaminfo.autoupdate.value and update:
			self.loop = eTimer()
			self.loop.callback.append(update)
			self.loop.start(config.oscaminfo.intervall.value * 1000, False)
		else:
			if self.loop:
				self.loop.stop()

	def keyPage_Down(self):
		self["outlist"].pageUp()
	...
	def key_menu(self):
		self.session.openWithCallback(boundFunction(self.handleAutoupdate, self.updateOScamData), OScamInfoSetup)

	def key_blue(self):
		self.handleAutoupdate()
		self.session.openWithCallback(self.onLayoutFinished, OScamShowLog)
...
class OScamShowLog(Screen, OScamGlobals):
	...
	def onLayoutFinished(self):
		self.handleAutoupdate(self.displayLog)
	...
	def handleAutoupdate(self, update=None):
		if config.oscaminfo.autoupdateFull.value and update:
			self.loop = eTimer()
			self.loop.callback.append(update)
			self.loop.start(config.oscaminfo.intervallFull.value * 1000, False)
		else:
			if self.loop:
				self.loop.stop()
