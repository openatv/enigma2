...
class RecordTimerEntry(TimerEntry):
	...
	def activate(self):
		...
		if nextState == self.StatePrepared:
			...
			# our escalation by 5 seconds without incrementing first_try_prepare.
			...
			if self.first_try_prepare == 0:
				...
				if simultaneousPrepared:
					self.log(8, "Simultaneous timer '%s' already prepared; deferring tuner-release escalation by 5 seconds." % simultaneousPrepared[0].name)
					self.start_prepare = int(time()) + 5
					return False
