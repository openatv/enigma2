...
def SymbolsCheck(session, **kwargs):
		...
		if getBoxType() in ('force3uhd','wetekplay', 'wetekplay2', 'ixussone', 'ixusszero', 'mbmicro', 'e4hd', 'e4hdhybrid', 'dm7020hd', 'dm7020hdv2', '9910lx', '9911lx') or getMachineBuild() in ('dags7362' , 'dags73625', 'dags5'):
			POLLTIME = 1
	...
	def Recording(self):
		if fileExists("/proc/stb/lcd/symbol_circle"):
			...
			if recordings > 0:
				...
				open("/proc/stb/fp/led_set", "w").write("0xffffffff")
		elif getMachineBuild() in ('dags7362' , 'dags73625', 'dags5') or getBoxType() in ('force3uhd'):
			recordings = len(NavigationInstance.instance.getRecordings(False,Components.RecordingConfig.recType(config.recording.show_rec_symbol_for_rec_types.getValue())))
			self.blink = not self.blink
			if recordings > 0:
				if self.blink:
