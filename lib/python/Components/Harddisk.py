...
class HarddiskManager:
	...
	def enumerateBlockDevices(self):
		...
		# Synchronously unmount any stale /media/* mounts belonging to external mmcblk
		# devices left over from the previous boot before enigma2 started.
	...
	def _getCorrectMountpoint(self, device):
		"""Return the correct mountpoint for a device, remounting if necessary.
		   Never writes fstab - that is enumerateHotPlugDevices's responsibility.
		   Respects any existing fstab entries (user or auto-generated by enumerateHotPlugDevices)."""
		from os import listdir
		from os.path import realpath as oRealpath
		currentMount = self.getMountpoint(device)
		black = BoxInfo.getItem("mtdblack")
		# --- mmcblk devices ---
		if search(r"^mmcblk\d+p\d+$", device):
			dev = self.splitDeviceName(device)[0]
			# dm900/dm920 internal data partition
			if BoxInfo.getItem("model") in ("dm900", "dm920") and device == f"{black}p3":
				if currentMount is None or currentMount == "/":
					newFstab = fileReadLines("/etc/fstab", default=[], source=MODULE_NAME)
					if not any(f"/dev/{device}" in line for line in newFstab):
						newFstab.append(f"/dev/{device} /media/data ext4 rw,relatime,data=ordered 0 0")
						fileWriteLines("/etc/fstab", newFstab, source=MODULE_NAME)
					...
					return "/media/data/"
				return currentMount
			# Internal eMMC - leave alone
			if dev.startswith(black):
				return currentMount
			# External SD card - check fstab first
			newFstab = fileReadLines("/etc/fstab", default=[], source=MODULE_NAME)
			try:
				uuidLinks = listdir("/dev/disk/by-uuid") if exists("/dev/disk/by-uuid") else []
				uuid = next((u for u in uuidLinks if oRealpath(f"/dev/disk/by-uuid/{u}") == f"/dev/{device}"), None)
			except Exception:
				uuid = None
			inFstab = any(f"/dev/{device}" in line or (uuid and uuid in line) for line in newFstab)
			if inFstab:
				# Already in fstab - respect it, use current mount
				print(f"[Harddisk] MMC '{device}' already in fstab, leaving at '{currentMount}'.")
				return currentMount
			# Always run assignment logic to determine correct mountpoint
			# Use only self.partitions for used mountpoints - /proc/mounts has stale kernel mounts
			usedMounts = {p.mountpoint.rstrip("/") for p in self.partitions if p.mountpoint}
			hasSdDevice = any(search(r"^sd\w\d+$", p.device) for p in self.partitions if p.device)
			if hasSdDevice:
				possibleMountPoints = [f"/media/{x}" for x in ("mmc3", "mmc2", "mmc1", "mmc") if f"/media/{x}" not in usedMounts]
			else:
				possibleMountPoints = [f"/media/{x}" for x in ("mmc3", "mmc2", "mmc1", "mmc", "hdd") if f"/media/{x}" not in usedMounts]
			...
			if targetMount and currentMount != targetMount + "/":
				...
				# Unmount stale mounts of this device, but don't touch mountpoints
				# occupied by a different sd* device in /proc/mounts
				procMounts = getProcMountsNew()
				otherSdMounts = {mount[1] for mount in procMounts if mount[0].startswith("/dev/sd") and mount[0] != f"/dev/{device}"}
		...
		if search(r"^sd\w\d+$", device):
			...
			inFstab = any(f"/dev/{device}" in line or (uuid and uuid in line) for line in newFstab)
			if inFstab:
				# Already in fstab - respect it completely
			...
			if targetMount and currentMount != targetMount + "/":
				if not exists(targetMount):
					mkdir(targetMount, 0o755)
				# Unmount all instances of this device from /proc/mounts before remounting
				for mount in getProcMountsNew():
					if mount[0] == f"/dev/{device}" and mount[1].startswith("/media/"):
						self.console.ePopen(["/bin/umount", "/bin/umount", "-lf", mount[1]])
				self.console.ePopen(["/bin/mount", "/bin/mount", f"/dev/{device}", targetMount])
			return (targetMount + "/") if targetMount else currentMount
		# All other devices
