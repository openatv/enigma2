			...
			if config.usage.swap_time_display_on_osd.value == "1":
				if remaining is not None:
					if config.usage.swap_time_remaining_on_osd.value == "1": # Elapsed
						return sign_p + ngettext(_("%d Min"), _("%d Mins"), (old_div(p,60))) % (old_div(p,60))
					elif config.usage.swap_time_remaining_on_osd.value == "2": # Elapsed & Remaining
						return sign_p + "%d  " % (old_div(p,60)) + sign_r + ngettext(_("%d Min"), _("%d Mins"), (old_div(r,60))) % (old_div(r,60))
					elif config.usage.swap_time_remaining_on_osd.value == "3": # Remaining & Elapsed
						return sign_r + "%d  " % (old_div(r,60)) + sign_p + ngettext(_("%d Min"), _("%d Mins"), (old_div(p,60))) % (old_div(p,60))
					else:
						return sign_r + ngettext(_("%d Min"), _("%d Mins"), (old_div(r,60))) % (old_div(r,60))
				else:
					return ngettext(_("%d Min"), _("%d Mins"), (old_div(l,60))) % (old_div(l,60))
				...
				if remaining is not None:
					...
					return "%d:%02d" % (old_div(l,60), l%60)
				...
				if self.type == self.DEFAULT:
					...
					if remaining is not None:
						return ngettext(_("%d"), _("%d"), (old_div(r,60))) % (old_div(r,60))
