...
class FrontendInfo(Converter):
	...
	def __init__(self, type):
		...
		if type.startswith("STRING"):
			...
			type = type.split(",")
			self.spaceForTuners = len(type) > 1 and int(type[1]) or 10
			self.spaceForTunersWithSpaces = len(type) > 2 and int(type[2]) or 6
		...
		if self.type in (self.STRING_REC, self.REC_TUNER) and NavigationInstance.instance:
	...
	def getText(self):
		...
		match self.type:
			...
			case self.SNR | self.SNRdB:  # Fallback to normal SNR.
				percent = self.source.snr
			case self.STRING | self.STRING_REC:
				tuners = []
				isRec = self.type == self.STRING_REC
				recordingTuners = self.getRecordingTuners() if isRec else ()
				slots = [x for x in nimmanager.nim_slots if (x.enabled if isRec else x.type)]
				count = len(slots) if isRec else len(nimmanager.nim_slots)
				for slot in slots:
					if isRec and slot.slot in recordingTuners:
						color = r"\c00FF0000"
					elif slot.slot == self.source.slot_number:
						color = r"\c0000FF00"
