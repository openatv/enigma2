		...
		if self.type < 5:
			...
			if config.usage.swap_media_time_display_on_osd.value == "1":  # Mins
				if self.type == self.TYPE_LENGTH:
					return ngettext(_("%d Min"), _("%d Min"), (l/60)) % (l/60)
				elif self.type == self.TYPE_POSITION:
					if config.usage.swap_time_remaining_on_osd.value == "1": # Elapsed
						return sign_p + ngettext(_("%d Min"), _("%d Min"), (p/60)) % (p/60)
					elif config.usage.swap_time_remaining_on_osd.value == "2": # Elapsed & Remaining
						return sign_p + "%d  " % (p/60) + sign_r + ngettext(_("%d Min"), _("%d Min"), (r/60)) % (r/60)
					elif config.usage.swap_time_remaining_on_osd.value == "3": # Remaining & Elapsed
						return sign_r + "%d  " % (r/60) + sign_p + ngettext(_("%d Min"), _("%d Min"), (p/60)) % (p/60)
					else:
						return sign_r + ngettext(_("%d Min"), _("%d Min"), (r/60)) % (r/60)
