...
class ServicePosition(Poll, Converter, object):
	...
	def getText(self):
		...
		if self.type == self.TYPE_SUMMARY or self.type == self.TYPE_SUMMARY:
			s = old_div(self.position, 90000)
			e = (old_div(self.length, 90000)) - s
			return "%02d:%02d +%2dm" % (old_div(s,60), s%60, old_div(e,60))
		...
		if self.type < self.TYPE_VFD_LENGTH:
			...
			if config.usage.swap_media_time_display_on_osd.value == "1":  # Mins
				if self.type == self.TYPE_LENGTH:
					return ngettext("%d Min", "%d Mins", (old_div(l,60))) % (old_div(l,60))
				elif self.type == self.TYPE_POSITION:
					if config.usage.swap_time_remaining_on_osd.value == "1": # Elapsed
						return sign_p + ngettext("%d Min", "%d Mins", (old_div(p,60))) % (old_div(p,60))
					elif config.usage.swap_time_remaining_on_osd.value == "2": # Elapsed & Remaining
						return sign_p + "%d  " % (old_div(p,60)) + sign_r + ngettext("%d Min", "%d Mins", (old_div(r,60))) % (old_div(r,60))
					elif config.usage.swap_time_remaining_on_osd.value == "3": # Remaining & Elapsed
						return sign_r + "%d  " % (old_div(r,60)) + sign_p + ngettext("%d Min", "%d Mins", (old_div(p,60))) % (old_div(p,60))
					else:
						return sign_r + ngettext("%d Min", "%d Mins", (old_div(r,60))) % (old_div(r,60))
				...
				if self.type == self.TYPE_LENGTH:
						return sign_l + "%d:%02d" % (old_div(l,60), l%60)
				elif self.type == self.TYPE_POSITION:
					if config.usage.swap_time_remaining_on_osd.value == "1":  # Elapsed
						return sign_p + "%d:%02d" % (old_div(p,60), p%60)
					elif config.usage.swap_time_remaining_on_osd.value == "2": # Elapsed & Remaining
						return sign_p + "%d:%02d  " % (old_div(p,60), p%60) + sign_r + "%d:%02d" % (old_div(r,60), r%60)
					elif config.usage.swap_time_remaining_on_osd.value == "3": # Remaining & Elapsed
						return sign_r + "%d:%02d  " % (old_div(r,60), r%60) + sign_p + "%d:%02d" % (old_div(p,60), p%60)
					else:
						return sign_r + "%d:%02d" % (old_div(r,60), r%60)
				elif self.type == self.TYPE_REMAINING:
					if config.usage.swap_time_remaining_on_osd.value == "1":  # Elapsed
						return sign_p + "%d:%02d" % (old_div(p,60), p%60)
					elif config.usage.swap_time_remaining_on_osd.value == "2" or config.usage.swap_time_remaining_on_osd.value == "3": # Remaining & Elapsed
				...
				if self.type == self.TYPE_LENGTH:
					return sign_l + "%d:%02d" % (old_div(l,3600), old_div(l%3600,60))
				elif self.type == self.TYPE_POSITION:
					if config.usage.swap_time_remaining_on_osd.value == "1":  # Elapsed
						return sign_p + "%d:%02d" % (old_div(p,3600), old_div(p%3600,60))
					elif config.usage.swap_time_remaining_on_osd.value == "2": # Elapsed & Remaining
						return sign_p + "%d:%02d  " % (old_div(p,3600), old_div(p%3600,60)) + sign_r + "%d:%02d" % (old_div(r,3600), old_div(r%3600,60))
					elif config.usage.swap_time_remaining_on_osd.value == "3": # Remaining & Elapsed
						return sign_r + "%d:%02d  " % (old_div(r,3600), old_div(r%3600,60)) + sign_p + "%d:%02d" % (old_div(p,3600), old_div(p%3600,60))
					else:
						return sign_r + "%d:%02d" % (old_div(r,3600), old_div(r%3600,60))
				elif self.type == self.TYPE_REMAINING:
					if config.usage.swap_time_remaining_on_osd.value == "1":  # Elapsed
						return sign_p + "%d:%02d" % (old_div(p,3600), old_div(p%3600,60))
					elif config.usage.swap_time_remaining_on_osd.value == "2" or config.usage.swap_time_remaining_on_osd.value == "3": # Remaining & Elapsed
				...
				if self.type == self.TYPE_LENGTH:
					return sign_l + "%d:%02d:%02d" % (old_div(l,3600), old_div(l%3600,60), l%60)
				elif self.type == self.TYPE_POSITION:
					if config.usage.swap_time_remaining_on_osd.value == "1":  # Elapsed
						return sign_p + "%d:%02d:%02d" % (old_div(p,3600), old_div(p%3600,60), p%60)
					elif config.usage.swap_time_remaining_on_osd.value == "2": # Elapsed & Remaining
						return sign_p + "%d:%02d:%02d  " % (old_div(p,3600), old_div(p%3600,60), p%60) + sign_r + "%d:%02d:%02d" % (old_div(r,3600), old_div(r%3600,60), r%60)
					elif config.usage.swap_time_remaining_on_osd.value == "3": # Remaining & Elapsed
						return sign_r + "%d:%02d:%02d  " % (old_div(r,3600), old_div(r%3600,60), r%60) + sign_p + "%d:%02d:%02d" % (old_div(p,3600), old_div(p%3600,60), p%60)
					else:
						return sign_r + "%d:%02d:%02d" % (old_div(r,3600), old_div(r%3600,60), r%60)
				elif self.type == self.TYPE_REMAINING:
					if config.usage.swap_time_remaining_on_osd.value == "1":  # Elapsed
						return sign_p + "%d:%02d:%02d" % (old_div(p,3600), old_div(p%3600,60), p%60)
					elif config.usage.swap_time_remaining_on_osd.value == "2" or config.usage.swap_time_remaining_on_osd.value == "3": # Remaining & Elapsed
				...
				if not self.detailed:
						...
						if self.showHours:
							...
							if self.showNoSeconds:
								if self.type == self.TYPE_LENGTH:
									...
									return ngettext("%d", "%d", (old_div(r,60))) % (old_div(r,60))
								elif self.type == self.TYPE_REMAINING:
									return sign_r + ngettext("%d Min", "%d Mins", (old_div(r,60))) % (old_div(r,60))
								...
								if self.type == self.TYPE_LENGTH:
									...
									if config.usage.elapsed_time_positive_vfd.value:
										myRestMinuten = "%+6d" % (old_div(r,60))
									else:
										myRestMinuten = "%+6d" % (old_div(r,60)*-1)
									if (old_div(r,60)) == 0:
										myRestMinuten = " "
									time = getTime()
									t = localtime(time)
					...
					if self.showHours:
						if self.type == self.TYPE_LENGTH:
							return sign_l + "%d:%02d:%02d:%03d" % ((old_div(old_div(l,3600),90000)), old_div((old_div(l,90000))%3600,60), (old_div(l,90000))%60, old_div((l%90000),90))
						elif self.type == self.TYPE_POSITION:
							return sign_r + "%d:%02d:%02d:%03d" % ((old_div(old_div(r,3600),90000)), old_div((old_div(r,90000))%3600,60), (old_div(r,90000))%60, old_div((r%90000),90))
						elif self.type == self.TYPE_REMAINING:
							return sign_p + "%d:%02d:%02d:%03d" % ((old_div(old_div(p,3600),90000)), old_div((old_div(p,90000))%3600,60), (old_div(p,90000))%60, old_div((p%90000),90))
					else:
						if self.type == self.TYPE_LENGTH:
							return sign_l + "%d:%02d:%03d" % ((old_div(old_div(l,60),90000)), (old_div(l,90000))%60, old_div((l%90000),90))
						elif self.type == self.TYPE_POSITION:
							return sign_p + "%d:%02d:%03d" % ((old_div(old_div(p,60),90000)), (old_div(p,90000))%60, old_div((p%90000),90))
						elif self.type == self.TYPE_REMAINING:
							return sign_r + "%d:%02d:%03d" % ((old_div(old_div(r,60),90000)), (old_div(r,90000))%60, old_div((r%90000),90))
		else:
			if config.usage.elapsed_time_positive_vfd.value:
				sign_p = "+"
				sign_r = "-"
