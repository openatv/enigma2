...
class ServicePosition(Poll, Converter, object):
	...
	def getText(self):
		...
		if self.type < 5:
			...
			if config.usage.swap_media_time_display_on_osd.getValue() == "1":  # Mins
				if self.type == self.TYPE_LENGTH:
					...
					if config.usage.swap_time_remaining_on_osd.getValue() == "1": # Elapsed
						return sign_p + ngettext("%d Min", "%d Mins", (p/60)) % (p/60)
					elif config.usage.swap_time_remaining_on_osd.getValue() == "2" or config.usage.swap_time_remaining_on_osd.getValue() == "3": # Remaining & Elapsed
						return ""
					else:
						return sign_r + ngettext("%d Min", "%d Mins", (r/60)) % (r/60)
			elif config.usage.swap_media_time_display_on_osd.getValue() == "2":  # Mins Secs
				if self.type == self.TYPE_LENGTH:
					...
					if config.usage.swap_time_remaining_on_osd.getValue() == "1":  # Elapsed
						...
						return sign_r + "%d:%02d" % (r/60, r%60)
			elif config.usage.swap_media_time_display_on_osd.getValue() == "3":  # Hours Mins
				if self.type == self.TYPE_LENGTH:
					...
					if config.usage.swap_time_remaining_on_osd.getValue() == "1":  # Elapsed
						...
						return sign_r + "%d:%02d" % (r/3600, r%3600/60)
			elif config.usage.swap_media_time_display_on_osd.getValue() == "4":  # Hours Mins Secs
				if self.type == self.TYPE_LENGTH:
					...
					if config.usage.swap_time_remaining_on_osd.getValue() == "1":  # Elapsed
						...
						return sign_r + "%d:%02d:%02d" % (r/3600, r%3600/60, r%60)
			elif config.usage.swap_media_time_display_on_osd.getValue() == "5":  # Percentage
				if self.type == self.TYPE_LENGTH:
					return sign_l + "%d:%02d" % (l/3600, l%3600/60)
				elif self.type == self.TYPE_POSITION:
					if config.usage.swap_time_remaining_on_osd.getValue() == "1":  # Elapsed
					...
					if config.usage.swap_time_remaining_on_osd.getValue() == "1":  # Elapsed
						try:
							...
							return ""
					elif config.usage.swap_time_remaining_on_osd.getValue() == "2" or config.usage.swap_time_remaining_on_osd.getValue() == "3": # Elapsed & Remaining
						return ""
					else:
						try:
							return sign_r + "%d%%" % ((float(p + 0.0) / float(l + 0.0)) * 100)
			...
			if config.usage.elapsed_time_positive_vfd.getValue():
				...
				sign_p = "-"
				sign_r = "+"
			if config.usage.swap_media_time_display_on_vfd.getValue() == "1":  # Mins
				if self.type == self.TYPE_VFD_LENGTH:
					...
					if config.usage.swap_time_remaining_on_vfd.getValue() == "1": # Elapsed
						return sign_p + ngettext("%d Min", "%d Mins", (p/60)) % (p/60)
					elif config.usage.swap_time_remaining_on_vfd.getValue() == "2" or config.usage.swap_time_remaining_on_vfd.getValue() == "3": # Remaining & Elapsed
						return ""
					else:
						return sign_r + ngettext("%d Min", "%d Mins", (r/60)) % (r/60)
			elif config.usage.swap_media_time_display_on_vfd.getValue() == "2":  # Mins Secs
				if self.type == self.TYPE_VFD_LENGTH:
						return sign_l + "%d:%02d" % (l/60, l%60)
				elif self.type == self.TYPE_VFD_POSITION:
					if config.usage.swap_time_remaining_on_vfd.getValue() == "1":  # Elapsed
					...
					if config.usage.swap_time_remaining_on_vfd.getValue() == "1":  # Elapsed
						...
						return sign_r + "%d:%02d" % (r/60, r%60)
			elif config.usage.swap_media_time_display_on_vfd.getValue() == "3":  # Hours Mins
				if self.type == self.TYPE_VFD_LENGTH:
					return sign_l + "%d:%02d" % (l/3600, l%3600/60)
				elif self.type == self.TYPE_VFD_POSITION:
					if config.usage.swap_time_remaining_on_vfd.getValue() == "1":  # Elapsed
					...
					if config.usage.swap_time_remaining_on_vfd.getValue() == "1":  # Elapsed
						...
						return sign_r + "%d:%02d" % (r/3600, r%3600/60)
			elif config.usage.swap_media_time_display_on_vfd.getValue() == "4":  # Hours Mins Secs
				if self.type == self.TYPE_VFD_LENGTH:
					return sign_l + "%d:%02d:%02d" % (l/3600, l%3600/60, l%60)
				elif self.type == self.TYPE_VFD_POSITION:
					if config.usage.swap_time_remaining_on_vfd.getValue() == "1":  # Elapsed
					...
					if config.usage.swap_time_remaining_on_vfd.getValue() == "1":  # Elapsed
						...
						return sign_r + "%d:%02d:%02d" % (r/3600, r%3600/60, r%60)
			elif config.usage.swap_media_time_display_on_vfd.getValue() == "5":  # Percentage
				if self.type == self.TYPE_VFD_LENGTH:
					return sign_l + "%d:%02d" % (l/3600, l%3600/60)
				elif self.type == self.TYPE_VFD_POSITION:
					if config.usage.swap_time_remaining_on_vfd.getValue() == "1":  # Elapsed
					...
					if config.usage.swap_time_remaining_on_vfd.getValue() == "1":  # Elapsed
						try:
							...
							return ""
					elif config.usage.swap_time_remaining_on_vfd.getValue() == "2" or config.usage.swap_time_remaining_on_vfd.getValue() == "3": # Elapsed & Remaining
						return ""
					else:
						try:
							return sign_r + "%d%%" % ((float(p + 0.0) / float(l + 0.0)) * 100)
