...
#
	def buildTimerEntry(self, timer, processed):
		...
		res = [ None ]
		x = width / 2
		res.append((eListboxPythonMultiContent.TYPE_TEXT, self.iconWidth + self.iconMargin, 3, width, self.rowSplit, 0, RT_HALIGN_LEFT|RT_VALIGN_BOTTOM, gettimerType(timer)))
		if timer.timerType == TIMERTYPE.AUTOSTANDBY or timer.timerType == TIMERTYPE.AUTODEEPSTANDBY:
			...
			res.append((eListboxPythonMultiContent.TYPE_TEXT, 150, self.itemHeight - self.rowSplit, width-150, self.rowSplit, 1, RT_HALIGN_RIGHT|RT_VALIGN_BOTTOM, text))
			icon = None
			if not processed:
				if timer.state == TimerEntry.StateWaiting:
					state = _("waiting")
	...
	def __init__(self, list):
		...
		self.eventNameFont = gFont("Regular", 30)
		self.l.setList(list)
		self.itemHeight = 50
		self.rowSplit = 25
		self.iconMargin = 4
