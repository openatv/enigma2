...
class RunningText(Renderer):
	...
	def applySkin(self, desktop, screen):
		...
		if self.direction in (TOP,BOTTOM):
			...
			flh = int(fontRenderClass.getInstance().getLineHeight(self.txfont) or old_div(self.txfont.pointSize,6) + self.txfont.pointSize)
	...
	def calcMoving(self):
		...
		if self.lineHeight and self.direction in (TOP,BOTTOM):
			text_height = max(text_height, old_div((text_height + self.lineHeight - 1), self.lineHeight) * self.lineHeight)
		...
		if self.direction in (LEFT,RIGHT):
			...
			if self.type == RUNNING:
				...
				if text_width < self.W:
					...
					if text_width == self.W:
						text_width += max(2, old_div(text_width,20))
			...
			if self.type == RUNNING:
				...
				if text_height < self.H:
					...
					if text_height == self.H:
						text_height += max(2, old_div(text_height,40))
