...
const char* makefilename(const char* base, const char* ext, const char* post)
{
	std::string buf = "";
	if (base)
		buf = base;
	if(ext) {
    	std::string exts = ext;
        size_t extlen = exts.length();
        size_t buflen = buf.length();
        if(buflen >= extlen)
        {
            if (buf.rfind(exts.c_str(),buflen-extlen, extlen) == string::npos)
        		buf += ext;
        }
	}
...
int do_movie(char* inname)
{
	...
	struct stat fp;
	tmpname = makefilename(inname, ".ts", 0);
	...
	close(f_ap);
	close(f_sc);
	unlink(makefilename(inname, ".ts", ".reconstruct_apsc"));
	return 0;
	failure:
	if (f_ts != -1)
		close(f_ts);
	if (f_ap != -1) {
		close(f_ap);
		unlink(makefilename(inname, ".ts", ".ap"));
	}
	if (f_sc != -1) {
		close(f_sc);
		unlink(makefilename(inname, ".ts", ".sc"));
	}
	unlink(makefilename(inname, ".ts", ".reconstruct_apsc"));
	return 1;
}
