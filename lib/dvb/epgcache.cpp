				...
				case SHORT_EVENT_DESCRIPTOR:
				{
					...
					if( eventNameUTF8len > 0 ) // only store the data if there is something to store
					{
						/*
						 ...
						 previously some descriptors didn't match because their text was different and titles the same.
						 Now that we store them separately, we can save some space on title data;
						 some rough calculations show anywhere from 20 - 40% savings
						*/
						...
						title_len += 2; // add 2 to the length to include the 2 bytes in the header
...
#endif
			// remove this service's channel from the lastupdated map
			{
 ...
 * if its end time is earlier than @p eEPGCache::historySeconds ago.
 *
 * @return void
 */
...
void eEPGCache::load()
{
	...
	if (f == NULL)
	{
		/* no EPG on hard disk, so try internal flash */
...
#endif
		// write version string after binary data has been written to disk.
		fsync(fileno(f));
	...
	while(spos < argcount)
	{
		...
		switch((c=argstring[spos++]))
		{
			...
			case 'P': // Event Parental Rating
				...
				break;
			case 'W': // Event Content Description / Genre
...
//   W = Event Content Description ('W'hat) / Genre
//   C = Current Time
//   R = Service Reference
//   N = Service Name
//   n = Short Service Name
//   X = Return a minimum of one tuple per service in the result list... even when no event was found.
//       The returned tuple is filled with all available fields... unavailable are filled as None
//       The position and existence of 'X' in the format string has no influence on the result tuple, it's completely ignored.
...
//   the third:
//      when type is eventid, is the event_id
//      when type is time, is the start_time ( -1 for now_time )
//   the fourth is the end_time (optional), to query all events in the given time range
...
//		eDebug("[eEPGCache] have %d args('%s')", argcount, argstring);
	...
	while(listSize > listIt)
	{
		...
		if (PyTuple_Check(item))
		{
			...
			// redirect subservice queries to parent service
				...
				if (!startTimeQuery(ref, stime, minutes))
				{
					...
					while ( getNextTimeEntry(evt) != -1 )
					{
						if (forceReturnTen)  // GN return only 10 items
						{
							...
							{
								//eDebug("[eEPGCache] tuple entry no. 10 has been reached");
								break;
							}
							returnTenItemsCount++;
						}
	...
	const char* long_description, std::vector<uint8_t> event_types, std::vector<eit_parental_rating> parental_ratings, int source, uint16_t event_id)
{
	...
	*x = codePage;
	...
	for (int descrIndex = 0; descrIndex <= lastDescriptorNumber; ++descrIndex)
	{
		...
		// so the length field must be: stringlength + 1 (2 4-bits numbers) + 3 (lang code) + 2 bytes
		// for item info length field and text length field
...
/**
 * @brief Import EPG events from Python into the EPG database.
 * Each event in the @p list is added to each service in the @p serviceReferences list.
 *
...
//     0 = search for similar broadcasts (SIMILAR_BROADCASTINGS_SEARCH)
	...
	if (!descr.empty())
	{
		...
		while(cit != eventDB.end() && maxcount)
		{
			...
			for (timeMap::iterator evit(evmap.begin()); evit != evmap.end() && maxcount; ++evit)
			{
				...
				// check whether any of our descriptors are used by this event
				unsigned int cnt = 0;
			...
			if (first)
			{
				// now start at the first service in epgcache database ( only in SIMILAR BROADCASTING SEARCH )
				first=false;
				cit=eventDB.begin();
			}
			else
...
// TODO: check if this is still needed... it's been 15 years
