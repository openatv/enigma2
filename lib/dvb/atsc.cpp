...
MasterGuideTable::MasterGuideTable(const uint8_t * const buffer)
{
	...
	for (uint32_t i = 11; i < (uint32_t)descriptorsLoopLength + 11; i += buffer[i + 1] + 2)
	{
		descriptor(&buffer[i], SCOPE_SI);
	}
}
...
ATSCEvent::ATSCEvent(const uint8_t * const buffer)
{
	...
	for (i = 12 + titleLength; i < 12u + titleLength + (uint32_t)descriptorsLoopLength; i += buffer[i + 1] + 2)
		descriptor(&buffer[i], SCOPE_SI);
}
