...
void eDVBFrontend::calculateSignalQuality(int snr, int &signalquality, int &signalqualitydb)
{
	...
	else if (!strcmp(m_description, "BCM7356 DVB-S2 NIM (internal)")) // Gigablue
	{
	...
	else if (!strcmp(m_description, "BCM7362 DVB-S2 NIM (internal)")) // Gigablue
	{
		ret = (int)((((double(snr) / (65536.0 / 100.0)) * 0.1710) - 1.0000) * 100);
	}
	...
	else if (!strcmp(m_description, "GIGA DVB-S2 NIM (SP2246T)")) // Gigablue
	{
		ret = (int)((((double(snr) / (65536.0 / 100.0)) * 0.1710) - 1.0000) * 100);
	}
	else if (strstr(m_description, "GIGA DVB-C/T NIM (SP8221L)")) // GigaBlue
	{
		ret = (int)(snr / 75);
		ter_max = 1700;
	}
	else if (strstr(m_description, "GIGA DVB-C/T NIM (SI4765)")) // GigaBlue
	{
		ret = (int)(snr / 75);
		ter_max = 1700;
	}
	else if (strstr(m_description, "GIGA DVB-C/T2 NIM (SI4768)")) // GigaBlue
	{
		ret = (int)(snr / 75);
		ter_max = 1700;
	}
	else if (strstr(m_description, "GIGA DVB-C/T NIM (SI41652)")) // GigaBlue
	{
		ret = (int)(snr / 75);
		ter_max = 1700;
	}
...
//		eDebugNoSimulate("[eDVBFrontend] tuneLoop %d\n", m_sec_sequence.current()->cmd);
		...
		switch (m_sec_sequence.current()->cmd)
		{
			...
			case eSecCommand::SEND_DISEQC:
				...
				eDebugNoSimulateNoNewLineStart("[eDVBFrontend] tuner %d sendDiseqc: ", m_dvbid);
...
RESULT eDVBFrontend::prepare_sat(const eDVBFrontendParametersSatellite &feparm, unsigned int tunetimeout)
{
	...
	if (!res)
	{
		eDebugNoSimulate("[eDVBFrontend] frontend %d prepare_sat System %d Freq %d Pol %d SR %d INV %d FEC %d orbpos %d system %d modulation %d pilot %d, rolloff %d",
			m_dvbid,
			feparm.system,
			feparm.frequency,
			feparm.polarisation,
...
RESULT eDVBFrontend::prepare_terrestrial(const eDVBFrontendParametersTerrestrial &feparm)
{
	...
	eDebugNoSimulate("[eDVBFrontend] frontend %d tuning dvb-t to %d khz, bandwidth %d, modulation %d, inversion %d",
	m_dvbid,
	feparm.frequency,
	feparm.bandwidth,
//	feparm.code_rate_HP,
