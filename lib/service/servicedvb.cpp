...
int eDVBServicePlay::getInfo(int w)
{
	...
	case sAudioPID:
		if (m_dvb_service)
		{
			...
			if (apid != -1)
				return apid;
			apid = m_dvb_service->getCacheEntry(eDVBService::cDTSPID);
			if (apid != -1)
				return apid;
			apid = m_dvb_service->getCacheEntry(eDVBService::cAACPID);
			if (apid != -1)
				return apid;
			apid = m_dvb_service->getCacheEntry(eDVBService::cLPCMPID);
			if (apid != -1)
				return apid;
			apid = m_dvb_service->getCacheEntry(eDVBService::cDTSHDPID);
			if (apid != -1)
				return apid;
		}
...
int eDVBServicePlay::selectAudioStream(int i)
{
	...
	if (m_dvb_service && ((i != -1) || (program.audioStreams.size() == 1)
		...
		&& (m_dvb_service->getCacheEntry(eDVBService::cDTSPID) == -1)
		&& (m_dvb_service->getCacheEntry(eDVBService::cAACPID) == -1)
		&& (m_dvb_service->getCacheEntry(eDVBService::cLPCMPID) == -1)
		&& (m_dvb_service->getCacheEntry(eDVBService::cDTSHDPID) == -1)
		&& (m_dvb_service->getCacheEntry(eDVBService::cAACHEAPID) == -1))))
	{
		...
		m_dvb_service->setCacheEntry(eDVBService::cAACPID, apidtype == eDVBAudio::aAAC ? apid : -1);
		m_dvb_service->setCacheEntry(eDVBService::cDTSPID, apidtype == eDVBAudio::aDTS ? apid : -1);
		m_dvb_service->setCacheEntry(eDVBService::cLPCMPID, apidtype == eDVBAudio::aLPCM ? apid : -1);
		m_dvb_service->setCacheEntry(eDVBService::cDTSHDPID, apidtype == eDVBAudio::aDTSHD ? apid : -1);
